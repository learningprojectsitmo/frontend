...
export const DndMonitorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const triggerEvent = useCallback(
        (eventType: DndEventType, activeId: string, overId?: string) => {
    ...
    return <DndMonitorContext.Provider value={contextValue}>{children}</DndMonitorContext.Provider>;
};
...
export const LiveRegion: React.FC<{ announcement: string }> = ({ announcement }) => {
    ...
    return (
        <div aria-live="assertive" aria-atomic className="sr-only" role="status">
            {announcement}
        </div>
    );
};
