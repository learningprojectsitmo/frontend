...
import { Plus, MoreHorizontal, Pencil, Palette, Trash2, Check, X, GripVertical } from 'lucide-react';
...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
    ...
    const setDropDirectionForTask = useCallback((taskId: number, direction: 'top' | 'bottom' | null) => {
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <DropdownMenu open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                                    <DropdownMenuTrigger asChild>
                                        ...
                                        >
                                            <MoreHorizontal className={cn('h-4 w-4', colorStyle.text)}/>
                                        </button>
                                    </DropdownMenuTrigger>
                                        ...
                                        <div className="py-2 w-full">
                                            <div className="flex items-center justify-between px-2 py-1 text-sm">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm focus:text-accent-foreground">Выбрать цвет</span>
                                                </div>
                                                {/* Белый цвет */}
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        column.color === 'white' && 'ring-2 ring-blue-500 ring-offset-1'
                                                    )}
                                                    title="Белый"
                                                ...
                                                {Object.entries(columnColors).map(([colorName, styles]) => (
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            styles.header,
                                                            column.color === colorName && 'ring-2 ring-blue-500 ring-offset-1'
                                                        )}
                                                    />
                                                ))}
                                            </div>
                    ...
                    {sortedTasks.map((task) => (
                        <div
                            ...
                            className={cn(
                                ...
                                dropDirectionMap.get(task.id) === 'top' && 'border-t-2 border-t-blue-500 -mt-1',
                                dropDirectionMap.get(task.id) === 'bottom' && 'border-b-2 border-b-blue-500 -mb-1'
                            )}
                        >
                            <KanbanTask
                                task={task}
                    ...
                    {sortedTasks.length === 0 && (
                        <div
                            className={cn(
                                ...
                                isDragOver && isDraggingTask && 'border-2 border-blue-400 border-dashed bg-blue-50 text-blue-500'
                            )}
                        >
                            {isDragOver && isDraggingTask ? 'Отпустите для добавления задачи' : 'Нет задач'}
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="py-4">
                        ...
                        {column.tasks && column.tasks.length > 0 && (
                            <p className="mt-2 text-red-600">
                                Вместе с колонкой будет удалено {column.tasks.length} задач{column.tasks.length === 1 ? 'а' : ''}.
                                Это действие нельзя отменить.
                            </p>
                        )}
                    </div>
                    <div className="flex justify-end gap-2">
                        <Button variant="outline" size="hug36" onClick={() => setIsDeleteDialogOpen(false)}>
                            Отмена
                        </Button>
                        <Button variant="outline" size="hug36" onClick={handleDeleteConfirm} className="text-red-600 hover:text-red-700 hover:bg-red-50">
                            Удалить
                        </Button>
                    </div>
                </DialogContent>
