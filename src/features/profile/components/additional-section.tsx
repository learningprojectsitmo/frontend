...
function SectionActions({ onAdd }: { onAdd: () => void }) {
    return (
        <button onClick={onAdd} className="text-[13px] font-medium text-blue-500 hover:text-blue-700 transition-colors">
            + Добавить
        </button>
    );
}
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider">Портфолио</h3>
            ...
            {showForm && (
                <AddForm>
                    <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Название" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="URL" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowForm(false)} className="text-xs text-gray-500 px-3 py-1.5">Отмена</button>
                        <button onClick={handleAdd} disabled={!title.trim() || !url.trim() || createMutation.isPending}
...
function EducationSection({ items }: { items: EducationFull[] }) {
    ...
    const handleAdd = () => {
        ...
        createMutation.mutate(
            { institution: institution.trim(), faculty: faculty.trim(), degree: degree.trim(), years: years.trim() },
    ...
    return (
        <div className="border-t border-gray-100 pt-5">
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider">Образование</h3>
                {!showForm && <SectionActions onAdd={() => setShowForm(true)} />}
            </div>
            {items.length === 0 && !showForm ? (
                ...
                <div className="flex flex-col gap-4">
                    {items.map((item) => (
                        <div key={item.id} className="group flex justify-between">
                            <div>
                                <p className="text-sm font-semibold text-gray-900">{item.institution}</p>
                                <p className="text-xs text-gray-500 mt-0.5">{item.faculty}</p>
                                <p className="text-xs text-gray-400 mt-0.5">{item.years} &middot; {item.degree}</p>
                            </div>
                            <button onClick={() => deleteMutation.mutate(item.id)}
            ...
            {showForm && (
                <AddForm>
                    ...
                    <input value={years} onChange={(e) => setYears(e.target.value)} placeholder="Годы" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowForm(false)} className="text-xs text-gray-500 px-3 py-1.5">Отмена</button>
                        <button onClick={handleAdd} disabled={!institution.trim() || createMutation.isPending}
                            className="text-xs font-medium text-white bg-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-600 disabled:opacity-50">
                            {createMutation.isPending ? "..." : "Добавить"}
                        </button>
                    </div>
...
function LanguageSection({ items }: { items: LanguageFull[] }) {
    ...
    const handleAdd = () => {
        ...
        createMutation.mutate(
            ...
            { onSuccess: () => { setName(""); setLevel(""); setFlag(""); setShowForm(false); } },
        );
    };
    ...
    return (
        <div className="border-t border-gray-100 pt-5">
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-[13px] font-semibold text-gray-400 uppercase tracking-wider">Языки</h3>
                {!showForm && <SectionActions onAdd={() => setShowForm(true)} />}
            </div>
            {items.length === 0 && !showForm ? (
                ...
                <div className="flex flex-col gap-2">
                    {items.map((lang) => (
                        <div key={lang.id} className="flex items-center justify-between text-sm group">
                            ...
                            <button onClick={() => deleteMutation.mutate(lang.id)}
                                className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Icon name="trash" size={14} />
                            </button>
                        </div>
            ...
            {showForm && (
                <AddForm>
                    <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Язык" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <input value={level} onChange={(e) => setLevel(e.target.value)} placeholder="Уровень" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <input value={flag} onChange={(e) => setFlag(e.target.value)} placeholder="Флаг (🇷🇺)" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowForm(false)} className="text-xs text-gray-500 px-3 py-1.5">Отмена</button>
