...
export const LanguagesCard = ({ languages, isEditing, resumeId }: Props) => {
    ...
    const handleAdd = () => {
        ...
        createMutation.mutate(
            ...
            { onSuccess: () => { setName(""); setLevel(""); setShowForm(false); } },
        );
    };
        ...
        <div className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6">
            ...
            {isEditing && showForm && (
                <div className="mt-4 flex flex-col gap-2 p-3 bg-gray-50 rounded-xl">
                    ...
                    <input value={level} onChange={(e) => setLevel(e.target.value)} placeholder="Уровень (A1, B2, Родной...)" className="text-sm px-3 py-2 rounded-lg border border-gray-200" />
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowForm(false)} className="text-xs text-gray-500 px-3 py-1.5">Отмена</button>
                        <button onClick={handleAdd} disabled={!name.trim() || createMutation.isPending} className="text-xs font-medium text-white bg-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-600 disabled:opacity-50">
