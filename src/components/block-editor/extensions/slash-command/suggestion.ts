...
import type { SuggestionListHandle, SuggestionListProps } from "./suggestion-list";

type SuggestionType = Omit<SuggestionOptions<SlashCommandSuggestionItem>, "editor">;

export const defaultSlashCommandItems: SlashCommandSuggestionItem[] = [
    {
        command: ({ editor, range }) => {
            chainFocus(editor).deleteRange(range).toggleNode("paragraph", "paragraph").run();
        },
    ...
    {
        command: ({ editor, range }) => {
            chainFocus(editor).deleteRange(range).setNode("heading", { level: 1 }).run();
        },
    ...
    {
        command: ({ editor, range }) => {
            chainFocus(editor).deleteRange(range).setNode("heading", { level: 2 }).run();
        },
    ...
    {
        command: ({ editor, range }) => {
            chainFocus(editor).deleteRange(range).setNode("heading", { level: 3 }).run();
        },
    ...
    {
        command: ({ editor, range }) =>
            chainFocus(editor).deleteRange(range).toggleCodeBlock({ language: "plaintext" }).run(),
...
const updatePosition = async (editor: Editor, element: Element): Promise<void> => {
...
export const getSlashCommandSuggestion = (
    ...
    const items = customItems
        ? [
              ...customItems,
              ...defaultSlashCommandItems.filter((d) => !customItems.some((c) => c.id === d.id)),
          ]
        : defaultSlashCommandItems;

    return {
        items: ({ query }) =>
            items.filter((item) => item.keywords.some((k) => k.startsWith(query.toLowerCase()))),
        render: () => {
