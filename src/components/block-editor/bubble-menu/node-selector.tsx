...
const nodeItems: NodeItem[] = [
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).setParagraph().run(),
    ...
    {
        command: (e) =>
            (e.chain().focus() as unknown as NodeChain).toggleHeading({ level: 1 }).run(),
    ...
    {
        command: (e) =>
            (e.chain().focus() as unknown as NodeChain).toggleHeading({ level: 2 }).run(),
    ...
    {
        command: (e) =>
            (e.chain().focus() as unknown as NodeChain).toggleHeading({ level: 3 }).run(),
        icon: <HeadingIcon level={3} />,
        isActive: (s) => s.isHeading3,
        name: "Heading 3",
    },
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).toggleBulletList().run(),
        icon: DEFAULT_ICONS.slashBulletListIcon,
        isActive: (s) => s.isBulletList,
        name: "Bullet List",
    },
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).toggleOrderedList().run(),
        icon: DEFAULT_ICONS.slashOrderedListIcon,
        isActive: (s) => s.isOrderedList,
        name: "Numbered List",
    },
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).toggleTaskList().run(),
        icon: DEFAULT_ICONS.slashTaskListIcon,
        isActive: (s) => s.isTaskList,
        name: "To-do List",
    },
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).toggleBlockquote().run(),
        icon: DEFAULT_ICONS.slashBlockquoteIcon,
        isActive: (s) => s.isBlockquote,
        name: "Quote",
    },
    {
        command: (e) => (e.chain().focus() as unknown as NodeChain).toggleCodeBlock().run(),
...
export const NodeSelector = ({ editor }: { editor: Editor }) => {
    ...
    return (
        <div style={{ position: "relative" }}>
            <BubbleButton onMouseDown={(e) => e.preventDefault()} onClick={() => setOpen(!open)}>
            ...
            {open && (
                <>
                    ...
                    <BubbleDropdown>
                        ...
                        >
                            <BubbleDropdownIcon>{DEFAULT_ICONS.deleteIcon}</BubbleDropdownIcon>
                            <span>Delete</span>
                        </BubbleDropdownItem>
                    </BubbleDropdown>
                </>
