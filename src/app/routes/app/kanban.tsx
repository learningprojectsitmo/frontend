...
import { useBoard, useCreateTask, useUpdateTask, useDeleteTask, useMoveTask, useCreateColumn, useUpdateColumn, useDeleteColumn, useReorderColumns, useCreateSubtask, useUpdateSubtask, useDeleteSubtask } from '@/features/kanban/hooks/useKanban';
...
export const KanbanRoute = () => {
  ...
  const { isOpen, editingTask, targetColumnId, openCreateModal, openEditModal, closeModal } = useTaskModal();
  ...
  const handleTaskSubmit = useCallback((data: TaskFormData, columnId: number) => {
    const processedData = {
      title: data.title,
    ...
    if (editingTask) {
      ...
      const existingNewIds = new Set(
        newSubtasks.filter(s => s.id).map(s => s.id)
      );

      const toDelete = oldSubtasks.filter(s => !existingNewIds.has(s.id));

      const toUpdate = newSubtasks.filter(s => s.id).map(newSub => {
        const oldSub = oldSubtasksMap.get(newSub.id!);
        const hasChanges = oldSub?.title !== newSub.title || oldSub?.isCompleted !== newSub.isCompleted;
        return {
          id: newSub.id!,
          title: newSub.title,
        ...
        onSuccess: async () => {
          try {
            ...
            for (const subtask of toUpdate) {
              operations.push(updateSubtask.mutateAsync({
            ...
            for (let i = 0; i < toCreate.length; i++) {
              operations.push(createSubtask.mutateAsync({
                taskId: editingTask.id,
  ...
  }, [editingTask, createTask, updateTask, createSubtask, updateSubtask, deleteSubtask, closeModal, refetch]);
  ...
  const handleCreateColumn = useCallback((data: { name: string; color: string; wipLimit?: number }) => {
    createColumn.mutate({
      projectId,
      name: data.name,
      color: data.color,
  ...
  const handleRenameColumn = useCallback((columnId: number, newName: string) => {
    updateColumn.mutate({
      columnId,
      data: { name: newName }
    }, {
  ...
  const handleChangeColor = useCallback((columnId: number, color: string) => {
    updateColumn.mutate({
      columnId,
      data: { color }
    }, {
  ...
  const handleTaskMove = useCallback((taskId: number, targetColumnId: number, targetPosition: number) => {
    moveTask.mutate({
      taskId,
      data: {
  ...
  const handleReorderColumns = useCallback((columnOrders: { id: number; position: number }[]) => {
  ...
  }), [columns, isLoading, handleTaskMove, openEditModal, openCreateModal, handleDeleteTask, handleRenameColumn, handleChangeColor, handleDeleteColumn, handleReorderColumns]);
        ...
        <div className="mx-auto max-w-7xl p-6">
          ...
          <div className="overflow-x-auto">
            <div className="flex min-w-min gap-x-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-64 flex-shrink-0 rounded-lg border border-gray-200 bg-white p-2">
